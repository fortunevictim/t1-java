package ru.t1.apupynin.clientms.aspect;
...
public class HttpIncomeRequestLogAspect {
    ...
    @Value("${spring.application.name:client-ms}")
    private String serviceName;

    private static final String SERVICE_LOGS_TOPIC = "service_logs";

    @Before("@annotation(ru.t1.apupynin.clientms.annotation.HttpIncomeRequestLog)")
    public void logHttpIncomeRequest(JoinPoint joinPoint) {
        log.debug("Logging HTTP income request for method: {}", joinPoint.getSignature().toShortString());
        ...
        try {
            ...
            log.debug("HTTP income request logged - Method: {}, URI: {}", methodSignature, uri);
    ...
    private void sendToKafka(Map<String, Object> logMessage) {
        try {
            ...
            log.debug("Successfully sent HTTP income log to Kafka topic: {}", SERVICE_LOGS_TOPIC);
        } catch (Exception e) {
